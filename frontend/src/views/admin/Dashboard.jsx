  ...
  return (
    <div className="container mx-auto p-6 bg-gray-50 text-gray-800 rounded-2xl">
      <header className="mb-6">
        <h1 className="text-3xl font-serif tracking-tight text-gray-800">Admin Dashboard</h1>
        <p className="text-sm text-gray-600">Overview of SF BANK system metrics — premium view</p>
      </header>

      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {/* Card: Members */}
        <div data-aos="fade-up" className="flex items-center gap-4 bg-white rounded-2xl shadow-lg p-4 sm:p-5 border">
          ...
          <div>
            ...
            <div className="text-2xl font-bold text-gray-800">{formatShort(totals.members)}</div>
            <div className="text-sm text-gray-600 mt-1">Active contributors</div>
          </div>
        </div>

        {/* resource cards */}
        <div data-aos="fade-up" data-aos-delay="40" className="flex items-center gap-4 bg-white rounded-2xl shadow p-4 sm:p-5 border">
          <div className="w-12 h-12 rounded-full bg-green-400 flex items-center justify-center text-green-700 font-semibold">F</div>
          <div>
            <div className="text-xs text-gray-500">Food</div>
            <div className="text-xl font-bold text-gray-800">{formatShort(totals.food)}</div>
            <div className="text-sm text-green-600 mt-1">total collected</div>
          </div>
        </div>

        <div data-aos="fade-up" data-aos-delay="80" className="flex items-center gap-4 bg-white rounded-2xl shadow p-4 sm:p-5 border">
          <div className="w-12 h-12 rounded-full bg-yellow-100 flex items-center justify-center text-yellow-700 font-semibold">W</div>
          <div>
            <div className="text-xs text-gray-500">Wood</div>
            <div className="text-xl font-bold text-gray-800">{formatShort(totals.wood)}</div>
            <div className="text-sm text-yellow-600 mt-1">total collected</div>
          </div>
        </div>

        <div data-aos="fade-up" data-aos-delay="120" className="flex items-center gap-4 bg-white rounded-2xl shadow p-4 sm:p-5 border">
          <div className="w-12 h-12 rounded-full bg-green-500 flex items-center justify-center text-slate-700 font-semibold">S</div>
          <div>
            <div className="text-xs text-gray-500">Stone</div>
            <div className="text-xl font-bold text-gray-800">{formatShort(totals.stone)}</div>
            <div className="text-sm text-slate-600 mt-1">total collected</div>
          </div>
        </div>

        <div data-aos="fade-up" data-aos-delay="160" className="flex items-center gap-4 bg-white rounded-2xl shadow p-4 sm:p-5 border">
          <div className="w-12 h-12 rounded-full bg-amber-100 flex items-center justify-center text-amber-700 font-semibold">G</div>
          <div>
            <div className="text-xs text-gray-500">Gold</div>
            <div className="text-xl font-bold text-amber-600">{formatShort(totals.gold)}</div>
            <div className="text-sm text-amber-600 mt-1">total collected</div>
          </div>
        </div>

        {/* Weeks card with progress */}
        <div data-aos="fade-up" data-aos-delay="200" className="bg-white rounded-2xl shadow p-4 sm:p-5 border">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-gray-400">Weeks (avg per member)</div>
              <div className="text-2xl font-bold text-gray-800">{avgWeeks}/{TOTAL_WEEKS}</div>
            </div>
            <div className="text-sm text-gray-600">{weeksProgress}%</div>
          </div>
          <div className="mt-3 bg-gray-100 rounded-full h-2 overflow-hidden">
            <div className="bg-amber-400 h-2" style={{ width: `${weeksProgress}%` }} />
          </div>
          <div className="text-xs text-gray-500 mt-2">Total weeks across members: <span className="text-amber-600 font-medium">{totals.weeks}</span></div>
        </div>
      </section>
      ...
      <section className="mt-6">
        <div data-aos="fade-up" className="bg-white rounded-2xl shadow p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-gray-500">Showing {members.length} members</div>
          </div>
                  ...
                  {pagedMembers.map((m, idx) => (
                    <tr key={m.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 flex items-center">
                        <div className="w-9 h-9 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center mr-3 font-semibold">
                          {getInitials(m.name)}
                        </div>
                        <div className="font-medium text-gray-800">{m.name}</div>
                      </td>
                    <td className="px-4 py-3 text-center text-gray-700">{formatShort(m.food)}</td>
                    <td className="px-4 py-3 text-center text-gray-700">{formatShort(m.wood)}</td>
                    <td className="px-4 py-3 text-center text-gray-700">{formatShort(m.stone)}</td>
                    <td className="px-4 py-3 text-center text-amber-600">{formatShort(m.gold)}</td>
                    <td className="px-4 py-3 text-center text-gray-700">{(m.weeksPaid || 0)}/{TOTAL_WEEKS}</td>
                    <td className="px-4 py-3 text-right font-semibold text-amber-600">{formatShort((m.food || 0) + (m.wood || 0) + (m.stone || 0) + (m.gold || 0))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <div key={m.id} className="bg-gray-50 rounded-xl p-3 shadow-sm">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="font-medium text-gray-800">{m.name}</div>
                      <div className="text-xs text-gray-500">Weeks: {(m.weeksPaid || 0)}/{TOTAL_WEEKS}</div>
                    </div>
                  </div>
                  <div className="text-right font-semibold">{formatShort((m.food || 0) + (m.wood || 0) + (m.stone || 0) + (m.gold || 0))}</div>
                </div>
                <div className="mt-3 grid grid-cols-4 gap-2 text-xs text-gray-600">
                  <div className="text-center">
                    <div className="text-amber-600 font-semibold">{formatToMillions(m.food)}</div>
                    <div>Food</div>
                  </div>
                  <div className="text-center">
                    <div className="text-yellow-600 font-semibold">{formatToMillions(m.wood)}</div>
                    <div>Wood</div>
                  </div>
                  <div className="text-center">
                    <div className="text-slate-700 font-semibold">{formatToMillions(m.stone)}</div>
                    <div>Stone</div>
                  </div>
                  <div className="text-center">
                    <div className="text-amber-700 font-semibold">{formatToMillions(m.gold)}</div>
                    <div>Gold</div>
                  </div>
                </div>
              </div>
